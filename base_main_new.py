...
os.makedirs('results_filter_existing_method', exist_ok=True)
savename = '; '.join(x.replace('--', '') for x in sys.argv[1:])
with open(f'results_filter_existing_method/{opt.savename}.csv', 'a') as csv_file:
    writer = csv.writer(csv_file)
    writer.writerow(
        [savename, np.mean(accs1), np.std(accs1), np.max(accs1), np.min(accs1), np.mean(accs5), np.std(accs5),
         np.max(accs5), np.min(accs5)])
    csv_file.close()

### Save model information as pkl.
if opt.save_model != '':
    os.makedirs('stored_models', exist_ok=True)
    pickle.dump({'scores_1': scores_1, 'scores_5': scores_5, 'encodings': encodings},
                open(f'stored_models/{opt.save_model}_{opt.dataset}.pkl', 'wb'))