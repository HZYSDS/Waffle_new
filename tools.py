...
CUB_DIR = '/root/autodl-tmp/datasets/Datasets/cub200'  # REPLACE THIS WITH YOUR OWN PATH
EUROSAT_DIR = '/root/autodl-tmp/datasets/eurosat'  # REPLACE THIS WITH YOUR OWN PATH
PLACES365_DIR = '/root/autodl-tmp/datasets/places365'  # REPLACE THIS WITH YOUR OWN PATH
PETS_DIR = '/root/autodl-tmp/datasets/pets'  # REPLACE THIS WITH YOUR OWN PATH
FOOD101_DIR = '/root/autodl-tmp/datasets/food101'  # REPLACE THIS WITH YOUR OWN PATH
DTD_DIR = '/root/autodl-tmp/datasets/dtd'  # REPLACE THIS WITH YOUR OWN PATH
...
def load_gpt_descriptions(opt: argparse.Namespace, classes_to_load: List = None, mode: str = 'clip'):
    ...
    ### (Lazy - uses gpt descriptions) Use the default CLIP setup.
    if mode == 'clip':
        gpt_descriptions = {l: opt.label_before_text + wordify(l) + opt.label_after_text for l in
                            opt.label_to_classname}
    ...
    if mode in need_descr_modes:
        gpt_descriptions = {key: [structured_descriptor_builder(item, key) for item in class_descr_list] for
                            key, class_descr_list in gpt_descriptions.items()}
    ...
    if mode == 'waffle_and_gpt':
        ...
        # subsample for each class individually. This does result in slightly different descriptor distributions per class.
        for key in key_list:
            gpt_descriptions[key] = list(
                np.random.choice(gpt_descriptions[key], effective_waffle_count * 3, replace=False))
