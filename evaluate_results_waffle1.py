...
files = [
    'results_waffle1/baselines.csv',

    'results_waffle1/baselines_gpt.csv',

    'results_waffle1/waffleclip.csv',
...
for file in files:
    ...
    for row in out:
        ...
        if 'label_before_text' in coln:
            coln = coln.split('label_before_text')[0] + ''.join(
                ''.join(coln.split('label_before_text')[1:]).split(';')[1:])
...
with open('output_table_waffle1.txt', 'w') as f:
    f.write('\n'.join(table_data))
