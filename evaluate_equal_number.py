...
files = [
    'results_equal_number/equal_number_filtering_dclip.csv',
    'results_equal_number/equal_number_filtering_equal_number.csv',
    'results_equal_number/equal_number_random_selection_comparative.csv',
    'results_equal_number/equal_number_random_selection_dclip.csv',
]
...
for file in files:
    ...
    for row in out:
        ...
        if 'k' in coln:
            coln = re.sub(r'k=\d+; ', '', coln)
            coln = re.sub(r'shot=\d+; ', '', coln)
            coln = re.sub(r'reps=\d+; ', '', coln)
...
with open('output_table_equal_number.txt', 'w') as f:
    f.write('\n'.join(table_data))