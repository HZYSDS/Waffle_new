...
do
    ...
    if [[ -n $concept ]]; then
        python base_main.py --savename=${savename} --dataset=${dataset} --k=${k} --shot=${shot} --mode=filtered_dclip --reps=5 --model_size=${backbone} --label_before_text="A photo of a ${concept}: a "
    fi
done