...
from torchvision.datasets import ImageNet, EuroSAT, Food101, Flowers102, Places365, OxfordIIITPet, DTD, FGVCAircraft, \
    StanfordCars, CIFAR100, Caltech256, SUN397
...
parser.add_argument('--model_size', type=str, required=True, choices=tools.BACKBONES,
...
def load_dataset(name):
    ...
    transform = transforms.Compose([
        ...
        # RuntimeError: output with shape [1, W, H] doesn't match the broadcast shape [3, W, H]
        transforms.ToTensor(),
    ...
    if name == 'imagenet' or name == 'imagenetv2':
        ...
        dataset = FGVCAircraft(root=tools.FGVCAIRCRAFT_DIR, split='train', annotation_level='family',
                               transform=transform)
    elif name == 'cars':
        ...
        data_dir = pathlib.Path(os.path.join(tools.CALTECH256_DIR,'256_ObjectCategories'))
        ...
        data_dir = pathlib.Path(tools.SUN397_DIR)
        dataset = SUN397(data_dir, transform=transform, download=True)
        torch.manual_seed(1)
        train_size = int(0.8 * len(dataset))
        test_size = len(dataset) - train_size
        train_dataset, _ = random_split(dataset, [train_size, test_size])
...
def get_class_names(name):
    ...
    if name == 'imagenetv2':
        ...
        with open(f'descriptors/dclip_descriptors_modify/descriptors_{name}.json', 'rb') as file:
            data = json.load(file)
