...
parser.add_argument('--mode', type=str, required=True, choices=['comparative', 'dclip'],
                    help='Choose the operation mode.')
parser.add_argument('--dataset', type=str, required=True, choices=tools.DATASETS,
                    help='Dataset to generate descriptors.')
...
A: There are several useful visual features to tell the photo is a {target}, not a {similar}.\n{features}"""
    ...
    examples = [
        ("soccer stadium", "baseball stadium",
         ["goal posts", "soccer ball", "shorts uniform", "rectangular field", "corner flags"]),
        ("harbor", "beach", ["docks", "breakwaters", "fishing boats", "cargo ships", "cranes"]),
        ("river", "lake", ["movement of water", "ripples", "wave", "bank", "delta"]),
        ("apple pie", "waffle",
         ["a golden-brown, flaky crust", "apple pieces", "spiced apple mixture", "round and deep shape",
          "light to deep brown color"]),
        ("anthurium", "hibiscus",
         ["shiny, heart-shaped spathe", "spadix", "arrow-shaped leaves", "glossy or leather-like leaf texture",
          "green or white colored spathes"]),
        ("abyssinian", "russian blue",
         ["ticked coat pattern", "warm reddish or golden fur", "large, alert ears", "slender body shape",
          "vibrant green or gold eyes"]),
        ("black-footed albatross", "frigatebird",
         ["a large, thick bill", "larger, with a dark plumage", "long, narrow wings", "webbed feet",
          "white underwings"]),
        ("beer glass", "coffee mug", ["beer", "carbonation bubbles", "clear glass", "rim shape", "logos"]),
        ("Ford model T", "golf cart",
         ["metal body panels", "doors", "convertible roof", "larger, rubber tires", "Ford logo"]),
        ("electric guitar", "acoustic guitar",
         ["solid body", "magnetic pickups", "control knobs", "input jack", "amplifier"])
    ]

    candidates = [
        qa_template.format(target=target, similar=similar, features='\n'.join(f'- {feature}' for feature in features))
        for target, similar, features in examples]
